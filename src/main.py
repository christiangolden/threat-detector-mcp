...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f'logs/threat_analysis_{datetime.now().strftime("%Y%m%d")}.log'
        ),
    ],
)
            ...
            for ent2 in sentence_entities[i + 1 :]:
                ...
                if start_idx < end_idx:
                    ...
                    entity_graph.add_edge(
                        ent1["text"], ent2["text"], relationship=" ".join(between_words)
                    )

    return entities, relationships


def analyze_entity_threat_patterns(
    ...
    try:
        ...
        try:
            ...
            final_threat_score = min(
                1.0, threat_score + entity_analysis["entity_threat_score"]
            )

            # Combine all suspicious patterns
            all_patterns = (
                suspicious_patterns + entity_analysis["entity_threat_patterns"]
            )
