...
class Entity(db.Model):
    ...
    analysis_result_id = db.Column(
        db.Integer, db.ForeignKey("analysis_results.id"), nullable=False
    )
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    analysis_result = db.relationship(
        "AnalysisResult", backref=db.backref("entity_objects", lazy=True)
    )
...
class ThreatPattern(db.Model):
    ...
    analysis_result = db.relationship(
        "AnalysisResult", backref=db.backref("pattern_objects", lazy=True)
    )
