...
def test_threatening_message():
    ...
    response = client.post("/analyze", json={"text": "I will kill you and destroy everything"})
    assert response.status_code == 200
    data = response.json()
...
def test_ambiguous_message():
    ...
    response = client.post("/analyze", json={"text": "The company plans to destroy the old building"})
    assert response.status_code == 200
    data = response.json()
